...
const Navbar = () => {
  ...
  return (
    <nav className="w-screen fixed top-0 z-50 bg-[rgba(255,255,255,.8)] flex items-center justify-between p-3 px-10 md:px-20">
      ...
      <button onClick={handleToggle} className="flex flex-col relative w-10 h-5 z-[99]">
        <div
          className={`w-full transition-all duration-300 bg-slate-500 ${
            kondisi ? "rotate-0 top-0" : "rotate-45 bottom-0"
          } absolute h-[.10rem]`}
        ></div>
        <div
          className={`w-full transition-opacity duration-300 bg-slate-500 ${
  ...
  } w-10 h-10 rounded-full bg-[rgba(241,245,249,.9)] right-0 top-0 justify-center items-center z-40`}
></div>
      ...
      >
        <ul className="flex flex-col gap-4 justify-center items-center text-slate-600">
